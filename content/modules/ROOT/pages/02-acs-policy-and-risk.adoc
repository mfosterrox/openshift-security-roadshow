...
* Identify risk in your clusters
* Clone and enforce a deploy-time policy
* Create policies from Risk filters
...
Runtime process enforcement (for example, blocking `apk` in a running pod) is xref:06-acs-network-and-runtime.adoc[06 Network and Runtime Security]. Build-time gates with `roxctl image check` are xref:03-acs-cicd-and-automation.adoc[03 CI/CD].
...
Cleanup for this lab restores the frontend deployment after you finish.
...
Morgan needs runtime context on the breached `payment-gateway` pod before locking down process execution.
...
RHACS has been observing the payment gateway since the page. Let's review Process Discovery before we enforce anything.
...
.Step 2 — Open payment-gateway in Risk
[%collapsible]
====
. In RHACS, go to *Risk* and search for `payment-gateway`.
. Open the deployment in namespace `payment-gateway-{workshop_user_suffix}`.
====

.Step 3 — Review Process Discovery
[%collapsible]
====
. Select the *Process Discovery* tab.
. Review the observed process baseline (should include `httpd` workers and possibly `/bin/bash` or `exfil.sh` from the simulated attack).
====

.Step 4 — Note baseline lock state
[%collapsible]
====
Note whether the baseline is *unlocked*—RHACS is still in its observation window and learning normal behavior.
====

=== What you learned

* Process Discovery captures every process observed inside a container during the learning phase.
* The payment gateway baseline may include attacker tooling (`exfil.sh`) if the intrusion occurred before observation ended.
* Risk profiling connects runtime behavior to prioritization before enforcement.
...
RHACS does not simply surface vulnerabilities. You learned the various risk priority justifications RHACS provides, filtered and searched through these vulnerabilities, and enforced a deploy-time policy against the frontend image.

Have fun with xref:03-acs-cicd-and-automation.adoc[CI/CD]!
