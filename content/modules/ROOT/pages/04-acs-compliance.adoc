...
Many Showroom / roadshow clusters already have the Compliance Operator installed by instructor setup (`setup/rhacs-configure.sh`). Check first:
...
*If the operator pods are Running*, skip the OperatorHub install and sensor restart below and continue with *Managing the Compliance Coverage Feature* (or review the existing daily schedule named `acs-catch-all` under *Compliance → Schedules*).

*If the operator is not installed*, follow this procedure:
...
IMPORTANT: RHACS 4.11 removed the classic *Compliance → Dashboard* (Scan environment, NIST/HIPAA/PCI workload scores). Use *Compliance → Coverage* and *Compliance → Schedules* instead. Instructor setup already installs the Compliance Operator and creates a daily `acs-catch-all` schedule.

== Execute Your First Compliance Scan (RHACS 4.10 and earlier)

When installing RHACS 4.10 or earlier you can kick off a classic compliance scan so the legacy dashboard has data. Skip this section on RHACS 4.11.
...
NOTE: Skip this section on RHACS 4.11 (classic dashboard removed). Continue with *Managing the Compliance Coverage Feature*.
...
The audit log showed `cicd-pipeline-runner-sa` had exec permissions it should never have used in production. Map that gap to compliance controls.
...
The pipeline service account had edit plus exec. Which compliance standards flag excessive RBAC and missing least privilege?

=== Your task

include::partial$security-exercise-task-intro.adoc[]

.Step 1 — Review the compliance dashboard
[%collapsible]
====
. In RHACS, open *Compliance* and review the cluster compliance dashboard.
====

.Step 2 — Identify RBAC-related failing controls
[%collapsible]
====
. Note the overall compliance percentage and any failing controls related to RBAC or access management.
====

.Step 3 — Map the audit finding to compliance controls
[%collapsible]
====
. Cross-reference with the audit finding: the `container-executor` Role granted `pods/exec` to `cicd-pipeline-runner-sa`.
====

.Step 4 — Discuss least-privilege gaps
[%collapsible]
====
Discuss which controls would have flagged this before the breach:

* Excessive permissions on automation service accounts
* Lack of separation between CI/CD deploy and runtime exec capabilities
* Missing audit review for privileged subresource access
====

=== What you learned

* Compliance dashboards translate incident findings into control framework language.
* The payment gateway breach violated least-privilege principles enshrined in CIS and STIG profiles.
* Compliance monitoring complements audit forensics for post-incident remediation planning.
...
In this lab, you installed the Compliance Operator and set up a daily scan that will report against OpenShift CIS benchmarks. You also reviewed platform compliance in the RHACS Compliance Coverage view. 

On to xref:05-acs-notifications-api.adoc[Notifications, API, and Integrations]!
