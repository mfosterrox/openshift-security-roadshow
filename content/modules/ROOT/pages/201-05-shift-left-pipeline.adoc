...
Add Gatekeeper policy to require annotation `signed=ok` or actual cosign verification (see xref:201-11-admission-governance.adoc[201-11]) before allowing tag-based images.
...
Controls that matter: Tekton (or equivalent) gates, conditional sign tasks, digest pins, and admission that extends trust (signatures/SBOM) at deploy time.
