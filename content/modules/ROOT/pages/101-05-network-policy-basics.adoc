...
TIP: `Now using project "101-05-n-netpol-demo"`.
====
...
TIP: `oc get svc api` should show port 8080.
====
...
TIP: `oc get pods` should show both clients `Running`.
====
...
TIP: Echo should be a cluster IP, not empty.
====
...
TIP: `networkpolicy.networking.k8s.io/default-deny-ingress created`.
====
...
TIP: `POD-SELECTOR` should be `<none>` (selects all pods).
====
...
TIP: `networkpolicy.networking.k8s.io/allow-api-from-allowed created`.
====
...
TIP: You should see both `default-deny-ingress` and `allow-api-from-allowed`.
====
...
TIP: The project should show `Terminating`.
====
...
IMPORTANT: default-deny in sensitive namespaces, smallest allow set that keeps the app working, consistent app/role/tier/env labels, and validation of allow vs deny paths.

NOTE: policies select pods; once selected, only explicit allows pass. CNI enforces at the pod interface—not a substitute for node firewalls. Optional pattern notes below can help you design the next rules.
