...
.Clone openshift-days-ops-showroom
[%collapsible]
====
The vault-config-scripts live in this repo. Clone it on the bastion if you do not already have it.
...
TIP: Skip clone if the directory already exists.
====

.Run install-vault.sh
[%collapsible]
====
The installer deploys the HashiCorp Helm chart into `vault` with the Agent Injector enabled.
...
NOTE: prefer short-lived, identity-bound access over copied passwords. Lab cleanup uses the Vault workshop scripts plus `lab-cleanup.sh --module 201-05`.
