...
== Overview
Wire HashiCorp Vault (and optionally External Secrets Operator) so pods get secrets at runtime without baking static copies into Git or images—dev-mode Vault for learning only.

include::partial$console-access.adoc[]

**Duration:** ~25 minutes · **Audience:** Platform Engineers, Security Engineers, IT Operations
...
This lab tracks the https://github.com/validatedpatterns/layered-zero-trust[Layered Zero Trust Validated Pattern] pattern for secrets (Vault, optional External Secrets Operator).
...
*Expected:* `rhos-token=workshop-token-abc123` and `username=workshop-user` / `password=workshop-password`.
...
**Great job!** For zero-trust patterns, see the https://github.com/validatedpatterns/layered-zero-trust[Layered Zero Trust Validated Pattern] on GitHub.
