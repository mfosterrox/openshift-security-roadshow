...
Deeper correlation with Loki and RHACS is xref:201-07-audit-monitoring-correlation.adoc[201-07]. Compliance Operator scans are xref:101-10-security-compliance-addons.adoc[101-10].
...
.Pull matching lines from kube-apiserver audit.log
[%collapsible]
====
This reads `/var/log/kube-apiserver/audit.log` on the first control plane node. Give the exec a few seconds to land in the file.
...
oc adm node-logs "${MASTER}" --path=kube-apiserver/audit.log \
  ...
  | wc -l
----
TIP: Count greater than 0 means your exec (or a port-forward) is in the Kubernetes API audit file. If 0, wait and re-run, or try another master (`oc get nodes -l node-role.kubernetes.io/master`).
====
...
.Unique exec actors in this slice
[%collapsible]
====
[source,bash,subs="+macros,+attributes",role=execute]
----
...
Controls that matter: keep OpenShift audit on, know the three `/var/log/…-apiserver/` trees, aggregate immutably off the node (CIS intent of 1.2.23–1.2.25 plus log forwarding), correlate with RHACS/runtime in later modules, and alert on anomalous verbs for sensitive namespaces.

Quick facts: audit shows API intent; it complements (does not replace) runtime detection. Metadata logs prove the exec; RequestResponse may also capture the command. Confirm retention and SIEM forwarding before you need them. Next: xref:101-10-security-compliance-addons.adoc[101-10] grades those CIS rows (path, rotation, forwarding) as PASS/FAIL.
