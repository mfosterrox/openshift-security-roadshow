...
IMPORTANT: RHACS, like Kubernetes works on a key:value system primarily due to their flexibility and power in organizing, selecting, and managing resources but also because of the ability to set-based selectors and match objects based on a set of values (e.g., "in", "not in", "exists", "does not exist").
