...
*Expected:* Failing APK and GitOps-managed policies. If you created *Alpine Linux Package Manager in Image - Enforce Deploy* in xref:02-acs-policy-and-risk.adoc[02 Policy and Risk], it appears here as well.
...
[start=3]
. Delete a locally managed conflicting build policy if you created one earlier (skip if the name is not found):
...
In this lab, you reviewed a basic CI/CD use case that implemented build-time policy checks with `roxctl` and GitOps-managed policies. You then adjusted the pipeline secret so the scan could reach Central. 

Congrats! On to xref:04-acs-compliance.adoc[Compliance]!
