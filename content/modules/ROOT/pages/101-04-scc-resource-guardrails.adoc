...
[%collapsible]
====
The image pull can take a minute. Wait until the rollout gives up (the pod will not become Available) before reading logs. If you still see `ContainerCreating`, wait and re-run `oc logs`.
...
----
TIP: Expect `CrashLoopBackOff` with `PermissionError` (cannot bind port 80 as non-root). `ContainerCreating` means the image is still pulling—not the SCC result yet.
====
...
.`anyuid` is the trap — try it only to understand the cost
[%collapsible]
====
...
Pin the grant to this lab namespace. On the roadshow bastion the command often *succeeds* because your user is privileged. Wait for the new pod before `exec` or logs—otherwise you will still see `ContainerCreating`.
...
----
If the grant succeeded: `id -u` is `0` and the process can listen on 80. That restored root for whoever owns the pod next. If it was Forbidden, stay on restricted SCC—that is the outcome you want in production.
====
