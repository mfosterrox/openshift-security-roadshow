...
TIP: An OpenShift *project* is a namespace plus metadata/access controls; create one boundary per team/environment/app; namespaces can talk by default—restrict with NetworkPolicy.
...
TIP: `Now using project "101-02-demo"`.
====
...
TIP: Labels should include `security-tier=basic` and `environment=dev`.
====
...
TIP: `rolebinding.rbac.authorization.k8s.io/dev-team created`.
====
...
TIP: Hard limits should show 2 CPU request, 2Gi memory, 10 pods.
====
...
TIP: `Now using project "team-payments-dev"`.
====
...
TIP: All three labels should appear on the namespace.
====
...
TIP: `rolebinding.rbac.authorization.k8s.io/payments-devs created`.
====
...
TIP: `oc describe` should list the hard limits from the table.
====
