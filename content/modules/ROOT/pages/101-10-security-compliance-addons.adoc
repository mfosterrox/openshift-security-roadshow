...
SPO and RHACS still matter for runtime and image risk; this hour is the audit-log follow-up. Tailoring and bulk remediate is xref:201-07-compliance-tailoring.adoc[201-07]. Log shipping into Loki is xref:201-06-audit-monitoring-correlation.adoc[201-06].
...
* **audit-log-forwarding-enabled** — logs stay on the masters until Cluster Logging / Loki (or a SIEM) ships them off-cluster. That is the common roadshow FAIL. Fix: configure a `ClusterLogForwarder` (see xref:201-06-audit-monitoring-correlation.adoc[201-06]), not “copy files off by hand.”
