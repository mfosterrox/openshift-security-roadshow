...
ACME has hundreds of deployments. Morgan wants every process baseline in the payment gateway namespace locked before end of day—automated via the RHACS API.
...
Manual baseline locking won't scale. Let's use the bulk lock endpoint for the whole namespace.

=== Your task

include::partial$security-exercise-task-intro.adoc[]

.Step 1 — Confirm API credentials
[%collapsible]
====
Ensure `ROX_API_TOKEN` and `ROX_CENTRAL_ADDRESS` are set (from earlier in this module).
====

.Step 2 — Retrieve the cluster ID
[%collapsible]
====
...
.Step 3 — Bulk-lock process baselines
[%collapsible]
====
Bulk-lock all process baselines in your payment gateway namespace:

[source,bash,subs="+macros,+attributes",role=execute]
----
export NAMESPACE="payment-gateway-{workshop_user_suffix}"
----

[source,bash,subs="+macros,+attributes",role=execute]
----
export CLUSTER_ID="<your-cluster-id-from-above>"
...
TIP: Expected response: `{"success": true}`.
====

.Step 4 — Verify baselines are locked
[%collapsible]
====
Verify in RHACS *Risk* that baselines for `payment-gateway` show as locked.
====

=== What you learned

* The RHACS API automates runtime security operations at namespace scale.
* Bulk baseline locking is practical for incident-driven hardening sprints.
* API automation complements the manual UI workflow from modules 03 and 04.
