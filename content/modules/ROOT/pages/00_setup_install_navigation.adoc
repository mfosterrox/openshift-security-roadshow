...
In the final step we will get access to the OpenShift cluster and various CLI tools via the bastion host or *"Bastion Host"*.

IMPORTANT: The default terminal in the Showroom environment should be the *Bastion Host*. The commands below will allow you to ssh into the bastion host should you lose connection.

NOTE: Please proceed to the "OpenShift admin access verification" section if you already have access to the *Bastion Host*
...
=== Download the "Java app" application and push the container image to Quay via the Student VM
