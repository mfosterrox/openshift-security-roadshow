...
|Zero Trust Workload Identity Manager (ZTWIM) | SPIFFE SVIDs bound to namespace and ServiceAccount; `spiffe-helper` renews them in the pod | East-west mTLS and workload identity, not the public Route. Hands-on is xref:301-08-zero-trust-workload-identity.adoc[301-08].
|===
...
Service-to-service mTLS with cert-manager is xref:201-08-cert-manager-mtls.adoc[201-08] (that lab also points a Certificate at this cluster’s ACME ClusterIssuer so a Route hostname is trusted without `-k`).
